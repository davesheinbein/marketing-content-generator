...
export default function PricingPlans({ onStripe, onPayPal }) {
  return (
    <div className="grid md:grid-cols-3 gap-6">
      {plans.map(plan => (
        <div key={plan.name} className="bg-white shadow rounded p-6 flex flex-col items-center">
          <h2 className="text-2xl font-bold mb-2">{plan.name}</h2>
          <div className="text-3xl text-blue-600 mb-2">{plan.price}</div>
