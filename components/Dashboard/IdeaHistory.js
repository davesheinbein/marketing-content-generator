		...
		<section className='mt-8'>
			<div className='mb-2 flex items-center justify-between'>
				...
				{!isPro && (
					<span className='text-sm text-gray-500'>
						Daily quota: {quota.used}/{quota.limit}
					</span>
				)}
			</div>
			{ideas.length === 0 && (
			...
			<ul className='grid gap-4'>
				{ideas.map((idea) => (
					<li
						key={idea.id}
						className='bg-white dark:bg-gray-800 rounded shadow p-5 flex flex-col gap-2'
					>
						<div className='font-semibold text-blue-700 dark:text-blue-200'>
							{idea.prompt}
						</div>
						<div className='whitespace-pre-line text-gray-700 dark:text-gray-100'>
							{idea.ideas}
						</div>
