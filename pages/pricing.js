...
export default function PricingPage() {
  ...
  const handlePayPal = (paypalPlanId) => {
    ...
    window.open(`https://www.paypal.com/checkoutnow?plan_id=${paypalPlanId}`, "_blank");
  };

  return (
    <Layout>
      <h1 className="text-3xl font-bold mb-8">Upgrade Your Plan</h1>
      <PricingPlans onStripe={handleStripe} onPayPal={handlePayPal} />
      {loading && <div className="mt-6 text-center text-blue-500">Redirecting to payment...</div>}
    </Layout>
  );
}